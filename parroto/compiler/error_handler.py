...
class CompilerErrorHandler(object):
    ...
    def SynErr(self, errNum, errPos=None):
        line, col = errPos if errPos else (
            self.parsing_position_handler() if self.parsing_position_handler else ("", 0))
    ...
    def Summarize(self, source_buffer):
        if self.is_merged:
            self.merged_file.write(self.TextAroundErrors(source_buffer.lines))
            self.merged_file.write(self.CountErrorFormat(see_log=False))
            self.merged_file.close()
        sys.stdout.write(self.TextAroundErrors(source_buffer.lines, lines_before=self.lines_before_msg,
                                               lines_after=self.lines_after_msg))
